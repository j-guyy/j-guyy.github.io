...
// ---- Baked creature art (real pixel-art PNGs) ----------------------------
// Species with a baked sprite at images/creatures/<id>.png / <id>_back.png
// (generated by game/tools/gen_sprites.py) use it; species without one —
// e.g. a new/edited species from the Creature Editor that hasn't had art
// generated for it yet — fall back to the procedural placeholder above.
// Front is the Pokedex/party/menu view and the battle opponent view; back is
// the player's own active creature in battle (viewed from behind, classic
// style). Call preloadCreatureArt() once at boot and await it before any
// spriteFor()/spriteBackFor() call that should see baked art; the baked*
// lookups themselves are synchronous cache reads so every other call site
// (Dex.spriteFor, evolveMember, ...) stays unchanged.

const bakedImages = {};
const bakedBackImages = {};

function loadOne(cache, id, src) {
  return new Promise((resolve) => {
    ...
    img.onerror = () => resolve(); // no baked art for this species — placeholder fallback
    img.src = src;
  });
}

export function preloadCreatureArt(speciesIds) {
  return Promise.all(
    speciesIds.flatMap((id) => [
      loadOne(bakedImages, id, `images/creatures/${id}.png`),
      loadOne(bakedBackImages, id, `images/creatures/${id}_back.png`),
    ])
  );
}

export function bakedCreatureSprite(speciesId) {
  const img = bakedImages[speciesId];
  return img ? new Sprite(img) : null;
}

export function bakedCreatureSpriteBack(speciesId) {
  const img = bakedBackImages[speciesId];
  return img ? new Sprite(img) : null;
}
